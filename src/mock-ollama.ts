...
/** Thrown by {@link MockOllama} when `invoke` is called with no responses queued. */
...
/**
 * In-memory test double for {@link Ollama}.
 *
 ...
 * @example
 * ```ts
 * const llm = new MockOllama({ text: 'hi', toolCalls: [], stopReason: 'end' })
 * ```
 */
